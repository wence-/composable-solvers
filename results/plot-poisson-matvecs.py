...
colours = seaborn.color_palette(n_colors=4)
...
for dim in dataframe["dimension"].drop_duplicates():
    ...
    for typ in sorted(dataframe["type"].drop_duplicates()):
        ...
        grouped = sliced.groupby(["degree"], as_index=False).mean()
        grouped["assemble_dof_per_second"] = (grouped.rows/grouped.assemble_time)
        grouped["matvec_dof_per_second"] = (grouped.rows/grouped.matmult_time)
        ...
        dimstr = {2: "2D",
                  3: "3D"}[dim]
        if typ != "matfree":
            ax.plot(grouped.degree, grouped.assemble_dof_per_second,
        ...
        ax.plot(grouped.degree, grouped.matvec_dof_per_second,
...
fig.savefig("poisson-matvec-time.pdf",
...
fig.savefig("poisson-matvec-memory.pdf",
            orientation="landscape",
            format="pdf",
            transparent=True,
            bbox_inches="tight",
